...
class CifCaf(Generator):
    ...
    def __call__(self, fields, initial_annotations=None):
        ...
        # Model may have 3 heads (cif, caf, caf25) with some outputs None; decoder needs 2 valid (cif, caf)
        non_none = [f for f in fields if f is not None]
        if len(fields) > 2 and len(non_none) >= 2:
            fields = [non_none[0], non_none[1]]
        elif len(fields) >= 2 and (fields[0] is None or fields[1] is None) and len(non_none) >= 2:
            fields = [non_none[0], non_none[1]]

        if self.field_config.cif_visualizers:
            for vis, cif_i in zip(self.field_config.cif_visualizers, self.field_config.cif_indices):
                if cif_i < len(fields) and fields[cif_i] is not None:
        ...
        if self.field_config.caf_visualizers:
            for vis, caf_i in zip(self.field_config.caf_visualizers, self.field_config.caf_indices):
                if caf_i < len(fields) and fields[caf_i] is not None:

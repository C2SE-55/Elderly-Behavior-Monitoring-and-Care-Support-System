...
def factory_decode(head_nets, *,
    ...
    # Model with 3 heads (cif, caf, caf25): use first two for CifCaf decoder
    if head_names == ('cif', 'caf', 'caf25'):
        head_nets = head_nets[:2]
        head_names = ('cif', 'caf')
    ...
    # Fallback when meta types don't match (e.g. checkpoint from openpifpaf): match by head names
    if head_names in (('cif', 'caf'), ('cif', 'caf', 'caf25')):
        head_nets = head_nets[:2]
        field_config = FieldConfig()
        field_config.cif_strides = [head_nets[0].stride(basenet_stride)]
        field_config.caf_strides = [head_nets[1].stride(basenet_stride)]
        skeleton = head_nets[1].meta.skeleton
        field_config.cif_visualizers = [
            visualizer.Cif(head_nets[0].meta.name,
                           stride=head_nets[0].stride(basenet_stride),
                           keypoints=head_nets[0].meta.keypoints,
                           skeleton=head_nets[0].meta.draw_skeleton)
        ]
        field_config.caf_visualizers = [
            visualizer.Caf(head_nets[1].meta.name,
                           stride=head_nets[1].stride(basenet_stride),
                           keypoints=head_nets[1].meta.keypoints,
                           skeleton=skeleton)
        ]
        return CifCaf(

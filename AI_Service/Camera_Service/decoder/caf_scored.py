...
class CafScored:
    ...
    def fill(self, fields):
        for caf_i, stride, min_distance, max_distance in zip(
            ...
            if caf_i >= len(fields) or fields[caf_i] is None:
                continue
            self.fill_caf(fields[caf_i], stride,
                          min_distance=min_distance, max_distance=max_distance)
        if self.forward is None:
            self.forward = [np.empty((9, 0), dtype=np.float32) for _ in self.skeleton]
            self.backward = [np.empty((9, 0), dtype=np.float32) for _ in self.skeleton]
        return self

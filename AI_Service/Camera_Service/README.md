...
## Stream API (cho Frontend – Giám sát và phát hiện hành vi)

Để video từ camera được **quét tự động và liên tục** bởi model té ngã và hiển thị trên Frontend tại `http://localhost:8081/camera`:

1. **Cài thêm (nếu chưa có):**  
   `pip install fastapi uvicorn`

2. **Chạy API** (chọn một trong hai):
   - Từ thư mục **AI_Service**:  
     `python -m Camera_Service.api.main`
   - Hoặc từ thư mục **Camera_Service**:  
     `python run_stream_api.py` (script tự chạy từ AI_Service)

   Hoặc dùng uvicorn từ AI_Service:  
   `uvicorn Camera_Service.api.main:app --host 0.0.0.0 --port 9000`

3. **Endpoint:**
   - `GET /stream` – MJPEG stream (video liên tục có overlay pose + phát hiện té ngã)
   - `GET /api/status` – JSON `{ fallcount, fps, ready }` để app hiển thị số lần té và FPS

4. Frontend (màn Camera Live) sẽ tự kết nối tới `http://localhost:9000` (hoặc host/port cấu hình trong app), hiển thị stream và cập nhật fall count theo thời gian thực.
